...
def plot_forces(data):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=frames, y=fx, mode='lines', name=f'Fx', line=dict(color='blue')))
    fig.add_trace(go.Scatter(x=frames, y=fy, mode='lines', name=f'Fy', line=dict(color='green')))
    fig.add_trace(go.Scatter(x=frames, y=fz, mode='lines', name=f'Fz', line=dict(color='red')))

    fig.update_layout(
        #title=dict(text='Forces', font=dict(size=24, family='Arial', color='black', weight='bold')),
        xaxis_title=dict(text='Frame', font=dict(size=18, family='Arial', color='black', weight='bold')),
        yaxis_title=dict(text='Force [N]', font=dict(size=18, family='Arial', color='black', weight='bold')),
        ...
        legend=dict(
            font=dict(size=18, family='Arial', color='black', weight='bold')  # Enlarge and bold the legend
        ),
        width = 1500,
        height = 800
    )
